...
import { FaUser, FaPhone, FaLock, FaShieldHalved, FaCamera, FaCircleCheck, FaTriangleExclamation, FaKey, FaBuilding, FaCity, FaIdCard, FaCar, FaTruck } from "react-icons/fa6";
import { useState, useRef, useEffect } from 'react';
import toast from 'react-hot-toast';
import axios from 'axios';

export default function DriverSettings({ user: propUser, governorates: propGovernorates }) {
    const { props } = usePage();
    const user = propUser || props.auth.user;
    const governorates = propGovernorates || props.governorates || [];
    ...
    useEffect(() => {
        if (user) {
            ...
            setSelectedGovernorateId(user?.governorate_id || '');
            console.log('User data loaded:', user);
        }
    }, [user]);

    // Set initial selected governorate
    useEffect(() => {
        if (user?.governorate_id) {
            setSelectedGovernorateId(user.governorate_id);
            
            // Load regions for the user's governorate
            const userGovernorate = governorates.find(gov => gov.id == user.governorate_id);
            if (userGovernorate && userGovernorate.regions) {
                setRegions(userGovernorate.regions);
            }
        }
    }, [user, governorates]);

    // Effect to update regions when a governorate is selected
    useEffect(() => {
        console.log('Selected governorate changed:', selectedGovernorateId);
        if (selectedGovernorateId) {
            const selectedGov = governorates.find(gov => gov.id == selectedGovernorateId);
            if (selectedGov && selectedGov.regions) {
                console.log('Setting regions:', selectedGov.regions);
                setRegions(selectedGov.regions);
            } else {
                setRegions([]);
            }
        } else {
            console.log('No governorate selected, clearing regions');
            setRegions([]);
        }
    ...
    const handleProfileUpdate = (e) => {
        e.preventDefault();
        profileForm.post('/driver/settings/profile', {
            onSuccess: () => {
    ...
    const requestPhoneOtp = async () => {
        ...
        try {
            await axios.post('/driver/settings/send-otp', {
    ...
    const verifyPhoneChange = (e) => {
        e.preventDefault();
        phoneForm.post('/driver/settings/verify-phone', {
            onSuccess: () => {
    ...
    const handleDeleteAccount = (e) => {
        e.preventDefault();
        deleteForm.delete('/driver/settings', {
    ...
    const sections = [
        ...
        { id: 'vehicle', name: 'معلومات المركبة', icon: FaCar, description: 'المركبة، اللوحة، النوع' },
        { id: 'location', name: 'الموقع والنشاط', icon: FaBuilding, description: 'المحافظة، المنطقة، نوع النقل' },
    ...
    return (
        <AuthenticatedLayout
            user={user}
            header="إعدادات الكابتن"
        >
            <Head title="إعدادات الكابتن" />
            ...
            <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
                ...
                <div className="space-y-6">
                    <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 text-center">
                        ...
                        <h3 className="text-xl font-bold text-gray-900">{user?.name || 'مستخدم'}</h3>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-widest">{user?.phone || ''}</p>
                        <div className="mt-3 inline-flex items-center px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-bold">
                            <FaTruck className="ml-1" />
                            {user?.transport_type === 'internal' ? 'نقل داخلي' : 'نقل خارجي'}
                        </div>
                    </div>
                            ...
                            {sections.map((section) => (
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[10px] text-gray-400 mt-1">{section.description}</p>
                                    </div>
                                </button>
                            ))}
                        </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {activeTab === 'profile' && (
                        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 animate-in fade-in slide-in-from-left-4 duration-300">
                            ...
                            <form onSubmit={handleProfileUpdate} className="space-y-6">
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2 mr-1">الرقم الوطني</label>
                                    <div className="relative">
                                        <FaIdCard className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" />
                                        <input
                                            type="text"
                                    ...
                                    {profileForm.errors.national_id && <p className="text-red-500 text-xs mt-2 mr-1">{profileForm.errors.national_id}</p>}
                                </div>
                    ...
                    {activeTab === 'vehicle' && (
                        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 animate-in fade-in slide-in-from-left-4 duration-300">
                            <h3 className="text-xl font-bold text-gray-900 mb-8 flex items-center">
                                <FaCar className="ml-3 text-primary" />
                                معلومات المركبة
                            </h3>

                            <form onSubmit={handleProfileUpdate} className="space-y-6">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2 mr-1">لوحة المركبة</label>
                                    <div className="relative">
                                        <FaIdCard className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" />
                                        <input
                                            type="text"
                                            value={profileForm.data.license_plate}
                                            onChange={e => profileForm.setData('license_plate', e.target.value)}
                                            className="w-full pr-12 pl-4 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary transition-all font-medium"
                                            placeholder="مثال: أ ب 1234"
                                        />
                                    </div>
                                    {profileForm.errors.license_plate && <p className="text-red-500 text-xs mt-2 mr-1">{profileForm.errors.license_plate}</p>}
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-2 mr-1">نوع المركبة</label>
                                        <div className="relative">
                                            <FaTruck className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" />
                                            <input
                                                type="text"
                                                value={profileForm.data.car_type}
                                                onChange={e => profileForm.setData('car_type', e.target.value)}
                                                className="w-full pr-12 pl-4 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary transition-all font-medium"
                                                placeholder="مثال: تويوتا"
                                            />
                                        </div>
                                        {profileForm.errors.car_type && <p className="text-red-500 text-xs mt-2 mr-1">{profileForm.errors.car_type}</p>}
                                    </div>

                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-2 mr-1">موديل المركبة</label>
                                        <div className="relative">
                                            <FaCar className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" />
                                            <input
                                                type="text"
                                                value={profileForm.data.car_model}
                                                onChange={e => profileForm.setData('car_model', e.target.value)}
                                                className="w-full pr-12 pl-4 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary transition-all font-medium"
                                                placeholder="مثال: كامري 2020"
                                            />
                                        </div>
                                        {profileForm.errors.car_model && <p className="text-red-500 text-xs mt-2 mr-1">{profileForm.errors.car_model}</p>}
                                    </div>
                                </div>
                    ...
                    {activeTab === 'location' && (
                        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 animate-in fade-in slide-in-from-left-4 duration-300">
                            ...
                            <form onSubmit={handleProfileUpdate} className="space-y-6">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2 mr-1">نوع النقل</label>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div
                                            onClick={() => profileForm.setData('transport_type', 'internal')}
                                            className={`cursor-pointer border-2 rounded-2xl p-4 flex flex-col items-center transition-all ${profileForm.data.transport_type === 'internal' ? 'border-primary bg-yellow-50' : 'border-gray-200 hover:border-gray-300'}`}
                                        >
                                            <div className="font-bold text-gray-900">داخلي</div>
                                            <div className="text-xs text-center text-gray-500 mt-1">داخل المحافظة</div>
                                        </div>
                                        <div
                                            onClick={() => profileForm.setData('transport_type', 'external')}
                                            className={`cursor-pointer border-2 rounded-2xl p-4 flex flex-col items-center transition-all ${profileForm.data.transport_type === 'external' ? 'border-primary bg-yellow-50' : 'border-gray-200 hover:border-gray-300'}`}
                                        >
                                            <div className="font-bold text-gray-900">خارجي</div>
                                            <div className="text-xs text-center text-gray-500 mt-1">بين المحافظات</div>
                                        </div>
                                    </div>
                                    {profileForm.errors.transport_type && <p className="text-red-500 text-xs mt-2 mr-1">{profileForm.errors.transport_type}</p>}
                                </div>
